...
my $sequtil_pair1 = Transposome::SeqUtil->new( file => $pair1, format => $seq_format, sample_size => $sample_size, seed => 100, in_memory => 1 );
my $sequtil_pair2 = Transposome::SeqUtil->new( file => $pair2, format => $seq_format, sample_size => $sample_size, seed => 100, in_memory => 1 );
